 ...
 * - tf.image.resizeBilinear(): redimensiona para [INPUT_MODEL_DIMENSIONS, INPUT_MODEL_DIMENSIONS]
 * - .div(255): normaliza os valores para [0, 1]
 * - .expandDims(0): adiciona dimensão batch [1, H, W, 3]
 *
 * Uso de tf.tidy():
 * - Garante que tensores temporários serão descartados automáticamente,
...
async function runInference(tensor) {
  ...
  // Assume que as 3 primeiras saídas são: caixas (boxes),
  // pontuações(scores) e classes
...
/**
 * Filtre e processa as predições:
 * - Aplica o limiar de confiança (CLASS_THRESHOLD)
 * - Filtra apneas a clase desejada (exemplo: 'kite)
 * - Converte coordenadas normalizadas para pixels reais
 * - Calcula o centro do bouding box
 *
 * Uso de generator (function*):
 * _Permite enviar cada predição assim que processada, sem criar lista intermediária
 */
function *processPrediction({boxes, scores, classes}, width, height) {
  for (let index = 0; index < scores.length; index++){
    ...
    const centerX = x1 + boxWidth;
    const centerY = y1 + boxHeight;
